      ...
      <template #default>
        ...
        <div class="wechat-pay-confirm" v-else-if="qrcodeUrl">
          ...
          <div class="buyInfoWrap">
            ...
            <div class="foot">
              <div
                v-if="props.isLegal && !orderInvaildFlag"
              ...
              >
                <div>
                  ...
                  <div class="item">
                    <span>{{ $t("waitTolong") }}</span>
                    <span class="toIndex" @click="toIndex">{{
                      $t("goOnCheckOther")
                    }}</span>
                    <span>{{ $t("laterToMyNft") }}</span>
                  </div>
        ...
        <div class="pay-confirm" v-else>
              ...
              <div class="cover">
                <Cover
                  ...
                  :cover="[
                    ...
                    nft.val?.nftBackIcon
                      ? $filters.metafile(nft.val?.nftBackIcon)
                      : undefined,
                  ]"
                ></Cover>
              </div>
                ...
                <div class="user-list">
                  <div class="user-item flex flex-align-center">
                    <UserAvatar
                      ...
                      :type="nft.val?.nftIssueAvatarType"
                    ></UserAvatar>
                  ...
                  <div class="user-item flex flex-align-center">
                    <UserAvatar
                      :metaId="nft.val?.nftOwnerMetaId"
                      :type="nft.val?.nftOwnerAvatarType"
                    ></UserAvatar>
          ...
          <div class="total-price flex flex-align-center">
            ...
            <div class="amount flex flex-align-center">
              <img v-if="!props.isLegal && !cnyMode" :src="BsvIcon" /><span v-if="cnyMode"
                >￥</span
...
import PayModeDialog from "@/components/payModeDialog/payModeDialog.vue";
...
import {
  ...
  GetLegalNftDetail,
  GetLegalBuyNftInfo,
  createNftOrderForLegal,
  GetBlindBoxSaleDetail,
  wxcoreForBuyBlindBox,
  realBuyBlindBox,
  BuyBlindBox,
  BuyBlindBoxStatus,
  SendRawTx,
...
import { checkSdkStatus, getUserBuyExtraFee } from "@/utils/util";
...
import wechatIcon from "@/assets/images/wechatIcon.svg?url";
...
import _Result from "element-plus/lib/components/result";
...
const nftSalePrice = computed(() => {
  if (props.isLegal) {
    ...
    if (cnyMode.value) {
      ...
      return new Decimal(nft.val.bsvPrice * buyAmount.value)
        .div(Math.pow(10, 8))
        .toNumber();
    }
  } else {
...
const blindBoxShouldPay = computed(() => {
  return new Decimal(nftSalePrice.value)
    .plus(new Decimal(platformFee.value))
    .mul(Math.pow(10, 2))
    .toString();
});
...
const shouldPayPrice = computed(() => {
  let tempPrice;
  if (props.price) {
    tempPrice = new Decimal(
      Math.ceil((+props.price.split(" ")[1] * buyAmount.value + +platformFee.value) * 100)
    ).toString();
  }
  return tempPrice;
});
...
const legalOriginalPrice = computed(() => {
  return (
    +nftSalePrice.value /
    (1 +
      new Decimal(fee.val?.platformPercentage)
        .plus(fee.val?.royaltyPercentage)
        .toNumber())
  );
});
...
const platformFee = computed(() => {
  if (props.isLegal) {
    let platFee = new Decimal(fee.val?.platformPercentage)
      .mul(legalOriginalPrice.value)
      .toNumber();
    return platFee < 0.01 ? 0.01 : platFee.toFixed(2);
  } else if (props.blindBoxPage) {
    if (cnyMode.value) {
      return new Decimal(+nftSalePrice.value * 0.06 * buyAmount.value).toFixed(2) !=
        "0.00"
      ...
      return new Decimal(+nftSalePrice.value * 0.06 * buyAmount.value)
        ...
        .toNumber() >
        1000 / 10 ** 8
        ? new Decimal(+nftSalePrice.value * 0.06 * buyAmount.value)
            .div(Math.pow(10, 8))
            .toNumber()
        : 1000 / 10 ** 8;
    }
    ...
    if (cnyMode.value && !props.blindBoxPage) {
      return new Decimal(
        ((+nftSalePrice.value * store.state.exchangeRate.feeRate) / 100).toFixed(2)
      ).toNumber() <= 0
        ? 0.01
        : ((+nftSalePrice.value * store.state.exchangeRate.feeRate) / 100).toFixed(2);
    } else {
...
const royaltyFee = computed(() => {
  if (props.isLegal) {
    let royalFee = new Decimal(fee.val?.royaltyPercentage)
      .mul(legalOriginalPrice.value)
      .toNumber();
...
function payTimeOver() {
  const duration = dayjs.duration(+dayjs(endTime.value) - +dayjs());
...
async function newQrcode() {
  ...
  QRCode.toCanvas(canvas, qrcodeUrl.value, (err: any) => {
    if (err) {
      console.log(err);
    } else {
      console.log("success");
...
function makeTx(wxCoreOrderId: string) {
  return async (res: makeTxRes) => {
...
async function checkUserAddress(metaid: string | string[]) {
  try {
...
async function confirmPay(payPlatform?: PayPlatform) {
  if (payPlatform) {
    payPlatformIndex.value = payPlatformList.findIndex(
      (item) => item.platform === payPlatform
    );
  }
    ...
    if (store.state.userInfo!.metaId === nft.val.nftOwnerMetaId) {
      ...
      if (
        ...
        if (props.isLegal) {
                ...
                if (res?.code === 200) {
                  ...
                  if (sendRes.code === 0) {
                    const checkOrderStatus = setInterval(async () => {
                      ...
                      if (
                        orderStatusRes.code === 0 &&
                        orderStatusRes.data === "success"
                      ) {
          ...
          if (cnyMode.value) {
            const address = store.state.userInfo.address
              ? store.state.userInfo.address
              : await checkUserAddress(store.state.userInfo.metaId);
            if (!address) {
